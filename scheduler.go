...
func (scheduler *Scheduler) Cancel(ID task.ID) error {
...
func (scheduler *Scheduler) Clear() {
	for ID, currentTask := range scheduler.tasks {
...
func (scheduler *Scheduler) registerTask(task *task.Task) {
	fmneta, err := scheduler.funcRegistry.Add(task.Func.Function)
	if err != nil {
		fmt.Println("Error while register functiong in registerTask")
	} else {
		fmt.Println(fmneta)
	}
	task.Func = fmneta
